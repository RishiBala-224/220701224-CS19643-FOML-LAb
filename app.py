...
with col2:
    if st.button("🔮 Predict My Heart Risk", use_container_width=True, key="predict_button_main"):
        ...
        # Plotting the prediction as a bar chart
        labels = ['No Disease', 'Disease']
        colors = ['#2ecc71', '#e74c3c']

        fig, ax = plt.subplots(figsize=(6, 4))
        bars = ax.bar(labels, probability, color=colors)
        ax.set_ylim(0, 1)
        ax.set_ylabel("Probability")
        ax.set_title("Cardiovascular Disease Prediction")
